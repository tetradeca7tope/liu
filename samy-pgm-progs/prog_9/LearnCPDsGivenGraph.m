  ...
  % for this node.
    ...
    for k = 1:K
      ...
      [P.clg(i).mu_y(k), P.clg(i).sigma_y(k)] = ...
        FitG( squeeze(dataset(:,i,1)), ClassProb(:,k) );
      [P.clg(i).mu_x(k), P.clg(i).sigma_x(k)] = ...
        FitG( squeeze(dataset(:,i,2)), ClassProb(:,k) );
      [P.clg(i).mu_angle(k), P.clg(i).sigma_angle(k)] = ...
        FitG( squeeze(dataset(:,i,3)), ClassProb(:,k) );
    end
    ...
    for k = 1:K
      par_idx = G(i, 2, k);
      par_vars = squeeze( dataset(:, par_idx, :) );
      
      [beta_y, P.clg(i).sigma_y(k)] = FitLG( ...
          squeeze(dataset(:,i,1)), par_vars, ClassProb(:,k) );
      P.clg(i).theta(k, 1:4) = [beta_y(4,:), beta_y(1:3,:)'];
      
      [beta_x, P.clg(i).sigma_x(k)] = FitLG( ...
          squeeze(dataset(:,i,2)), par_vars, ClassProb(:,k) );
      P.clg(i).theta(k, 5:8) = [beta_x(4,:), beta_x(1:3,:)'];
      
      [beta_a, P.clg(i).sigma_angle(k)] = FitLG( ...
          squeeze(dataset(:,i,3)), par_vars, ClassProb(:,k) );
