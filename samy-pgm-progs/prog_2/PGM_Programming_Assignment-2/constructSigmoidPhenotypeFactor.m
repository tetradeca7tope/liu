...
% num_alleles = length(alleleWeights);

phenotypeFactor = struct(...
  'var', [phenotypeVar, geneCopyVarOneList', geneCopyVarTwoList'], ...
  'card', [2, numel(alleleWeights{1}), numel(alleleWeights{2}), ...
           numel(alleleWeights{1}), numel(alleleWeights{2})], ...
  'val', []);

weights = [alleleWeights{1}, alleleWeights{2}];
% [allelesToGenotypes, genotypesToAlleles] = ...
%   generateAlleleGenotypeMappers(num_alleles);
assignments = IndexToAssignment(1:prod(phenotypeFactor.card), ...
                                phenotypeFactor.card);
phen_positive_allele_assigns = assignments(1:(end/2), 1:end-1);
phen_positive_allele_assigns(:,[2,4]) = phen_positive_allele_assigns(:,[2,4]) + 2;

additive_effects = weights(phen_positive_allele_assigns);
phen_positive_probs = computeSigmoid(sum(additive_effects, 2));
all_probs = [phen_positive_probs, (1 -phen_positive_probs)]';

phenotypeFactor.val = all_probs(:)';

% weights, allelesToGenotypes, genotypesToAlleles, assignments, phen_positive_allele_assigns,
% additive_effects,phen_positive_probs,
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
