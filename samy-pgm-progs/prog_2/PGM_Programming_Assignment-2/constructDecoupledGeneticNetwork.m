...
% We shall first fill the details in with the 1st & 2nd gene copies of the
% parents.

% First the parents
for i = 1:numPeople
  if pedigree.parents(i, 1) == 0  % if they are parents
    % Gene Copy 1
    factorList(i) = childCopyGivenFreqsFactor(alleleFreqs, i);
    
    % Gene Copy 2
    factorList(numPeople + i) = childCopyGivenFreqsFactor(alleleFreqs, ...
                                  numPeople + i);
  end
end

% then the children, given the copy genes
for i = 1:numPeople
  if pedigree.parents(i, 1) ~= 0  % if they are not parents
    p1 = pedigree.parents(i,1);
    p2 = pedigree.parents(i,2);
    
    % Gene Copy 1 from parent 1
    factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, ...
            p1, numPeople + p1);
    % Gene Copy 2 from parent 2
    factorList(numPeople + i) = childCopyGivenParentalsFactor(numAlleles, ...
            numPeople + i, p2, numPeople + p2);
  end
end

% Now construct the phenotype distros conditioned on the genotype distros
for i = 1:numPeople
