...
genotypeFactor = struct('var', [genotypeVar], ...
                        'card', [num_genotypes], ...
                        'val', []);
genotypeFactor.val = zeros(1, num_genotypes);
% genotypeFactor.val = (alleleFreqs(genotypesToAlleles(:,1)) .* ...
%                       alleleFreqs(genotypesToAlleles(:,2)))';

AtG_probs = alleleFreqs * alleleFreqs';
% AtG_probs,
for i = 1:num_genotypes
  genotypeFactor.val(i) = sum(sum(AtG_probs(allelesToGenotypes == i)));
end
