...
% allelesToGenotypes, genotypesToAlleles,
num_genotypes = size(genotypesToAlleles, 1);
...
genotypeFactor = struct( ...
  'var', [genotypeVarChild,genotypeVarParentOne,genotypeVarParentTwo], ...
  'card', num_genotypes * ones(1,3), ...
  'val', []);
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

for i = 1:num_genotypes
  % Tracking parent 1 in this loop
  p1_alleles = genotypesToAlleles(i,:);
  for j = 1:num_genotypes
    %Tracking parent 2 in this loop
    p2_alleles = genotypesToAlleles(j,:);
    child_geno_probs = zeros( num_genotypes ,1);
    
    % Now go through every combination of p`_alleles and p2_alleles and sum
    % it up in child_geno_probs accordingly.
    for ii = 1:2
      for jj = 1:2
        ch_genotype = allelesToGenotypes(p1_alleles(ii), p2_alleles(jj));
        % Add .25 to the corresponding genotype
        child_geno_probs(ch_genotype) = child_geno_probs(ch_genotype)+ .25;
      end
    end
    
    % Now finally place this in genoTypeFactor.val appropriately.
    genotypeFactor.val( 1, (i-1)*num_genotypes^2 + (j-1)*num_genotypes + 1 : ...
                        (i-1)*num_genotypes^2 + (j)*num_genotypes ) = ...
      child_geno_probs';
