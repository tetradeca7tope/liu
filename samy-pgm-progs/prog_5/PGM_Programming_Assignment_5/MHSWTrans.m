...
%     pairwise_factor = intersect(G.var2factors{q_list(i,1)}, ...
%                                 G.var2factors{q_list(i,2)});
%     var_card = F(pairwise_factor).card(1);
%     coherent_assignments = repmat([1:var_card]', 1, 2);
%     sum_f_uu = sum(GetValueOfAssignment(F(pairwise_factor), ...
%                                         coherent_assignments));
%     q_list(i, 3) = sum_f_uu / sum(F(pairwise_factor).val);                                  
% %     GetValueOfAssignment(F(pairwise_factor), coherent_assignments)
% %     pairwise_factor, var_card, coherent_assignments, sum_f_uu, q_list(i, 3),
%     
%   end
% end
...
% DEBUG
% G.q_list, q_keep_indx, q_list, selected_edges, SelEdgeMat, var2comp, num_cc,
% selected_cc, selected_vars, A(selected_vars), new_value, A_prop, LogR,
% new_value, old_value,
...
transition_log_ratio = log_QY_ratio + log_R_ratio;

% Compute the joint prob of all factors
acceptance_log_ratio = transition_log_ratio;
for i = 1:numel(F)
  acceptance_log_ratio = acceptance_log_ratio + ...
                log(GetValueOfAssignment(F(i), A_prop(F(i).var))) - ...
                log(GetValueOfAssignment(F(i), A(F(i).var)));
end

acceptance_ratio = exp(acceptance_log_ratio);
p_acceptance = min(1, acceptance_ratio);

%DEBUG
% log_QY_ratio, log_R_ratio, acceptance_ratio, p_acceptance, A_prop,
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
