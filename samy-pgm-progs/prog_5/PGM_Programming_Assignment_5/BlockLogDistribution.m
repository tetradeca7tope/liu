...
required_factors = unique([G.var2factors{V}]);

joint_factor = F(required_factors(1));
for i = 2:numel(required_factors)
  joint_factor = factorProduct(joint_factor, F(required_factors(i)));
end
required_variables = joint_factor.var;

assignments = repmat(A, d, 1);
assignments(:, V) = repmat([1:d]', 1, numel(V));

LogBS = GetValueOfAssignment(joint_factor, ...
                             assignments(:,required_variables));
LogBS = log(LogBS);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
