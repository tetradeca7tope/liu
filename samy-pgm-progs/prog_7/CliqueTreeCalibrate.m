...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:        
        sepset_var_cell = {unnormalizedMessages.var};
        not_sepsets = cellfun(@isempty, sepset_var_cell);
        not_sepsets = reshape(not_sepsets, N, N);

        [i1, i2] = find(not_sepsets == 0, 1);
        sepset = FactorProduct(unnormalizedMessages(i1, i2), ...
                               unnormalizedMessages(i2, i1));
        Z = sum(sepset.val);        
        logZ = log(Z);
    else
        logZ = 0;
